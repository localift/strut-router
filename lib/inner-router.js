...
const parseMethods = _require.parseMethods;

var _require2 = require("./validator");

const compileParams = _require2.compileParams;
const compilePath = _require2.compilePath;
        ...
        for (const sec of security) {
          ...
          switch (def.type) {
            case "apiKey":
              {
                if (def.in === "header") {
                  return yield _this2.secHandlers[def.name](ctx, ctx.header[def.name.toLowerCase()]);
                } else {
                  return yield _this2.secHandlers[def.name](ctx, ctx.query[def.name]);
                }
              }
            default:
  ...
  middleware() {
    ...
    return (() => {
      var ref = _asyncToGenerator(function* (ctx, next) {
        ...
        if (!(yield _this3.authenticate(match, ctx))) {
          debug("Auth failed");
